...
export const appProductReadiness: ToolcraftProductReadiness = {
  mode: "product",
  productName: "Word Tide",
  productSummary:
    "Typographic texture generator for brand system #2: readable words fill the canvas as an image-driven dither grid or along traced flow streamlines, with tone zones, shape masks, marker highlights, and editable-text SVG export.",
  requestedBehavior:
    "Generate brand textures made of whole readable words instead of literal lines: words arrange along currents and flow patterns, recreate an uploaded image's luminance like a legible dither, fill tone bands from separate word lists, stay inside uploaded SVG/alpha shapes, and export as SVG with editable or outlined text.",
};
...
export const appAcceptance: readonly ToolcraftComponentAcceptance[] = [
  controlAcceptance({
    automatedTestName: "render mode switches layout engines",
    browserTestName: "browser: render mode switches layout engines",
    componentType: "segmented",
    expectedObservable:
      "Switching between Flow and Dither swaps streamline word placement for the word grid.",
    fixture: "default word tide",
    id: "mode.render",
    optionCoverage: "each-visible-item",
    target: "mode.render",
    userAction: "Click the Dither segment.",
  }),
  controlAcceptance({
    automatedTestName: "seed changes product output",
    browserTestName: "browser: seed changes product output",
    componentType: "slider",
    expectedObservable: "Dragging Seed reshuffles word order, dropout, and streamline seeding.",
    fixture: "default word tide",
    id: "tide.seed",
    target: "tide.seed",
  ...
  controlAcceptance({
    actionCoverage: ["randomize-seed"],
    automatedTestName: "randomize changes product output",
    browserTestName: "browser: randomize changes product output",
    componentType: "actions",
    expectedObservable: "Clicking Randomize advances the seed and re-lays out the texture.",
    fixture: "default word tide",
    id: "tide.randomize",
    target: "tide.randomize",
    userAction: "Click Randomize.",
  }),
  controlAcceptance({
    automatedTestName: "word list changes product output",
    browserTestName: "browser: word list changes product output",
    componentType: "code",
    expectedObservable:
      "Editing the word list places the new tokens as whole readable words in the texture.",
    fixture: "default word tide",
    id: "words.list",
    target: "words.list",
    userAction: "Replace the word list with new tokens and commit.",
  }),
  controlAcceptance({
    automatedTestName: "word order changes product output",
    browserTestName: "browser: word order changes product output",
    componentType: "select",
    expectedObservable:
      "Sequential repeats the list in input order; Random draws a seeded shuffle of tokens.",
    fixture: "default word tide",
    id: "words.order",
    optionCoverage: "each-visible-item",
    target: "words.order",
    userAction: "Open Word order and choose Random.",
  }),
  controlAcceptance({
    automatedTestName: "source image drives ink lifecycle",
    browserTestName: "browser: source image drives ink lifecycle",
    componentType: "fileDrop",
    evidence: "media-lifecycle",
    expectedObservable:
      "Uploading a source image re-inks word opacity and weight from sampled luminance; the rotate and flip actions update the runtime media transform metadata and the renderer resamples the rotated/flipped ImageData; clear/remove returns to uniform noise ink; section or global reset removes the uploaded source media since no default exists.",
    fixture: "1920x1080 source photo",
    id: "media.sourceImage",
    target: "media.sourceImage",
    userAction: "Upload a photo, rotate it, flip it, then clear it and reset controls.",
  }),
  controlAcceptance({
    automatedTestName: "contrast changes product output",
    browserTestName: "browser: contrast changes product output",
    componentType: "slider",
    expectedObservable:
      "Dragging Source levels steepens the luminance-to-ink response with an AE-style crush.",
    fixture: "source photo loaded",
    id: "source.contrast",
    target: "source.contrast",
    userAction: "Drag the Source levels slider with a source image loaded.",
  }),
  controlAcceptance({
    automatedTestName: "invert flips ink reading",
    browserTestName: "browser: invert flips ink reading",
    componentType: "switch",
    expectedObservable: "Toggling Invert makes light image areas receive dense ink.",
    fixture: "source photo loaded",
    id: "source.invert",
    target: "source.invert",
    userAction: "Toggle Invert on.",
  }),
  controlAcceptance({
    automatedTestName: "fade maps ink to opacity",
    browserTestName: "browser: fade maps ink to opacity",
    componentType: "switch",
    expectedObservable:
      "Fade on ramps word opacity by ink; off draws every placed word at full strength.",
    fixture: "default word tide",
    id: "ink.fade",
    target: "ink.fade",
    userAction: "Toggle Fade off.",
  }),
  controlAcceptance({
    automatedTestName: "sparsity drops words from light areas",
    browserTestName: "browser: sparsity drops words from light areas",
    componentType: "slider",
    expectedObservable: "Dragging Sparsity up removes words, thinning light areas first.",
    fixture: "default word tide",
    id: "ink.sparsity",
    target: "ink.sparsity",
    userAction: "Drag the Sparsity slider up.",
  }),
  controlAcceptance({
    automatedTestName: "tone zones enable band word lists",
    browserTestName: "browser: tone zones enable band word lists",
    componentType: "switch",
    expectedObservable:
      "Enabling Tone Zones fills dark, mid, and light bands from their own word lists (canvas-height bands without an image).",
    fixture: "dither mode",
    id: "zones.enabled",
    target: "zones.enabled",
    userAction: "Switch Mode to Dither, then enable Tone Zones.",
  }),
  controlAcceptance({
    automatedTestName: "zone split moves band boundaries",
    browserTestName: "browser: zone split moves band boundaries",
    componentType: "rangeSlider",
    controlPartCoverage: ["rangeSlider.lower", "rangeSlider.upper"],
    expectedObservable:
      "Band split is hidden while Zones is off and becomes visible when Zones is on; dragging the lower thumb grows the dark band and the upper thumb grows the light band.",
    fixture: "dither mode with zones",
    id: "zones.split",
    target: "zones.split",
    userAction: "Enable Zones so Band split appears, then drag both thumbs.",
  }),
  controlAcceptance({
    automatedTestName: "dark zone words change product output",
    browserTestName: "browser: dark zone words change product output",
    componentType: "code",
    expectedObservable:
      "Dark words is hidden while Zones is off, becomes visible when Zones is on, and editing it places the new tokens in the darkest band.",
    fixture: "dither mode with zones",
    id: "zones.dark",
    target: "zones.dark",
    userAction: "Enable Zones so Dark words appears, then edit the list and commit.",
  }),
  controlAcceptance({
    automatedTestName: "mid zone words change product output",
    browserTestName: "browser: mid zone words change product output",
    componentType: "code",
    expectedObservable:
      "Mid words is hidden while Zones is off, becomes visible when Zones is on, and editing it places the new tokens in the mid band.",
    fixture: "dither mode with zones",
    id: "zones.mid",
    target: "zones.mid",
    userAction: "Enable Zones so Mid words appears, then edit the list and commit.",
  }),
  controlAcceptance({
    automatedTestName: "light zone words change product output",
    browserTestName: "browser: light zone words change product output",
    componentType: "code",
    expectedObservable:
      "Light words is hidden while Zones is off, becomes visible when Zones is on, and editing it places the new tokens in the lightest band.",
    fixture: "dither mode with zones",
    id: "zones.light",
    target: "zones.light",
    userAction: "Enable Zones so Light words appears, then edit the list and commit.",
  }),
  controlAcceptance({
    automatedTestName: "grid gap changes word packing",
    browserTestName: "browser: grid gap changes word packing",
    componentType: "slider",
    expectedObservable: "Dragging Gap changes spacing between words and rows in the grid.",
    fixture: "dither mode",
    id: "grid.gap",
    target: "grid.gap",
    userAction: "Drag the Gap slider in Dither mode.",
  }),
  controlAcceptance({
    automatedTestName: "overlap packs dark ink words",
    browserTestName: "browser: overlap packs dark ink words",
    componentType: "switch",
    expectedObservable:
      "Enabling Overlap lets words stack over each other in very dark ink areas for a denser dither.",
    fixture: "dither mode with source image",
    id: "grid.overlap",
    target: "grid.overlap",
    userAction: "Switch to Dither, upload a source image, then enable Overlap.",
  }),
  controlAcceptance({
    automatedTestName: "grid jitter offsets slots",
    browserTestName: "browser: grid jitter offsets slots",
    componentType: "slider",
    expectedObservable: "Dragging Jitter breaks the strict grid with seeded offsets.",
    fixture: "dither mode",
    id: "grid.jitter",
    target: "grid.jitter",
    userAction: "Drag the Jitter slider in Dither mode.",
  }),
  controlAcceptance({
    automatedTestName: "tone spacing tightens zones",
    browserTestName: "browser: tone spacing tightens zones",
    componentType: "slider",
    expectedObservable:
      "Tone spacing at center keeps uniform gaps; left tightens darker zones and right tightens lighter zones.",
    fixture: "dither mode",
    id: "grid.spacingBias",
    target: "grid.spacingBias",
    userAction: "Drag Tone spacing left, then right, in Dither mode.",
  }),
  controlAcceptance({
    automatedTestName: "spacing range changes tone gap strength",
    browserTestName: "browser: spacing range changes tone gap strength",
    componentType: "slider",
    expectedObservable: "Dragging Spacing range amplifies how much tone spacing tightens gaps.",
    fixture: "dither mode",
    id: "grid.spacingRange",
    target: "grid.spacingRange",
    userAction: "Drag Spacing range up while Tone spacing is off center.",
  }),
  controlAcceptance({
    automatedTestName: "flow pattern changes word streams",
    browserTestName: "browser: flow pattern changes word streams",
    componentType: "select",
    expectedObservable: "Choosing a different pattern changes streamline geometry under the words.",
    fixture: "default word tide",
    id: "flow.pattern",
    optionCoverage: "each-visible-item",
    target: "flow.pattern",
    userAction: "Open Pattern and choose Vortex.",
  }),
  controlAcceptance({
    automatedTestName: "flow direction changes product output",
    browserTestName: "browser: flow direction changes product output",
    componentType: "slider",
    expectedObservable: "Dragging Direction rotates the base current.",
    fixture: "default word tide",
    id: "flow.direction",
  ...
  controlAcceptance({
    automatedTestName: "flow scale changes product output",
    browserTestName: "browser: flow scale changes product output",
    componentType: "slider",
    expectedObservable: "Dragging Scale changes how tightly the field pattern repeats.",
    fixture: "default word tide",
    id: "flow.frequency",
  ...
  controlAcceptance({
    ...
    componentType: "slider",
    expectedObservable: "Dragging Swirl adds rotational bias to the word streams.",
    fixture: "default word tide",
    id: "flow.swirl",
    target: "flow.swirl",
  ...
  controlAcceptance({
    ...
    componentType: "slider",
    expectedObservable: "Dragging Turbulence distorts word streams with noise.",
    fixture: "default word tide",
    id: "flow.turbulence",
  ...
  controlAcceptance({
    automatedTestName: "paths edit mode enables canvas overlay",
    browserTestName: "browser: paths edit mode enables canvas overlay",
    componentType: "switch",
    expectedObservable: "Enabling Edit paths shows the spline overlay for direct manipulation.",
    fixture: "default word tide",
    id: "paths.editMode",
    target: "paths.editMode",
    userAction: "Toggle Edit paths on.",
  }),
  controlAcceptance({
    automatedTestName: "paths reach changes product output",
    browserTestName: "browser: paths reach changes product output",
    componentType: "slider",
    expectedObservable: "Dragging Reach changes how far path influence extends into the current.",
    fixture: "word tide with path",
    id: "paths.reach",
    target: "paths.reach",
    userAction: "Drag the Reach slider with a path present.",
  }),
  controlAcceptance({
    automatedTestName: "paths strength changes product output",
    browserTestName: "browser: paths strength changes product output",
    componentType: "slider",
    expectedObservable: "Dragging Strength changes how strongly paths bend nearby word streams.",
    fixture: "word tide with path",
    id: "paths.strength",
    target: "paths.strength",
    userAction: "Drag the Strength slider with a path present.",
  }),
  controlAcceptance({
    actionCoverage: ["add-path"],
    automatedTestName: "add path creates editable spline",
    browserTestName: "browser: add path creates editable spline",
    componentType: "actions",
    expectedObservable: "Clicking Add path creates a new active path for canvas editing.",
    fixture: "default word tide",
    id: "paths.addPath",
  ...
  controlAcceptance({
    ...
    automatedTestName: "delete path removes active spline",
    browserTestName: "browser: delete path removes active spline",
    componentType: "actions",
    expectedObservable: "Clicking Delete path removes the active path from the current.",
    fixture: "word tide with path",
    id: "paths.deletePath",
  ...
  controlAcceptance({
    automatedTestName: "density changes stream packing",
    browserTestName: "browser: density changes stream packing",
    componentType: "slider",
    expectedObservable: "Dragging Density changes how many word streams fill the canvas.",
    fixture: "default word tide",
    id: "streams.density",
  ...
  controlAcceptance({
    automatedTestName: "word gap changes stream spacing",
    browserTestName: "browser: word gap changes stream spacing",
    componentType: "slider",
    expectedObservable: "Dragging Word gap changes spacing between consecutive words on a stream.",
    fixture: "default word tide",
    id: "streams.wordGap",
    target: "streams.wordGap",
    userAction: "Drag the Word gap slider.",
  }),
  controlAcceptance({
    automatedTestName: "font size styles every word",
    browserTestName: "browser: font size styles every word",
    componentType: "slider",
    expectedObservable:
      "Dragging Font size rescales Tay Big Bird uppercase words in preview and export.",
    fixture: "default word tide",
    id: "type.fontSize",
    target: "type.fontSize",
    userAction: "Drag the Font size slider.",
  }),
  controlAcceptance({
    automatedTestName: "color count maps ink to palette",
    browserTestName: "browser: color count maps ink to palette",
    componentType: "slider",
    expectedObservable:
      "Dragging Color count switches between one and four brand ink colors across word density.",
    fixture: "default word tide",
    id: "colors.count",
    target: "colors.count",
    userAction: "Drag Color count from 1 to 4.",
  }),
  controlAcceptance({
    automatedTestName: "palette colors repaint words",
    browserTestName: "browser: palette colors repaint words",
    componentType: "color",
    expectedObservable: "Editing Deep Teal repaints words mapped to the darkest ink band.",
    fixture: "default word tide",
    id: "colors.color1",
    target: "colors.color1",
    userAction: "Pick a new Deep Teal color.",
  }),
  controlAcceptance({
    automatedTestName: "dark slate color repaints words",
    browserTestName: "browser: dark slate color repaints words",
    componentType: "color",
    expectedObservable:
      "Dark Slate appears when Color count is at least 2; editing it repaints the second ink band.",
    fixture: "default word tide",
    id: "colors.color2",
    target: "colors.color2",
    userAction: "Raise Color count to 2, then pick a new Dark Slate color.",
  }),
  controlAcceptance({
    automatedTestName: "capri color repaints words",
    browserTestName: "browser: capri color repaints words",
    componentType: "color",
    expectedObservable:
      "Capri appears when Color count is at least 3; editing it repaints the third ink band.",
    fixture: "default word tide",
    id: "colors.color3",
    target: "colors.color3",
    userAction: "Raise Color count to 3, then pick a new Capri color.",
  }),
  controlAcceptance({
    automatedTestName: "sky blue color repaints words",
    browserTestName: "browser: sky blue color repaints words",
    componentType: "color",
    expectedObservable:
      "Sky Blue appears when Color count is 4; editing it repaints the lightest ink band.",
    fixture: "default word tide",
    id: "colors.color4",
    target: "colors.color4",
    userAction: "Raise Color count to 4, then pick a new Sky Blue color.",
  }),
  controlAcceptance({
    automatedTestName: "highlight words mark matches",
    browserTestName: "browser: highlight words mark matches",
    componentType: "text",
    expectedObservable:
      "Entering a word from the list draws a marker rect behind every matching placed word.",
    fixture: "default word tide",
    id: "highlight.words",
    target: "highlight.words",
    userAction: "Type OCEAN into Highlights words and commit.",
  }),
  controlAcceptance({
    automatedTestName: "highlight color changes marker fill",
    browserTestName: "browser: highlight color changes marker fill",
    componentType: "color",
    expectedObservable: "Changing the marker color repaints highlight rects.",
    fixture: "highlight coverage active",
    id: "highlight.color",
    target: "highlight.color",
    userAction: "Set Coverage above zero, then pick a new marker color.",
  }),
  controlAcceptance({
    automatedTestName: "highlight coverage marks random words",
    browserTestName: "browser: highlight coverage marks random words",
    componentType: "slider",
    expectedObservable: "Dragging Coverage highlights a growing share of words at random.",
    fixture: "default word tide",
    id: "highlight.coverage",
    target: "highlight.coverage",
    userAction: "Drag the Coverage slider up.",
  }),
  controlAcceptance({
    automatedTestName: "shape mask constrains placement lifecycle",
    browserTestName: "browser: shape mask constrains placement lifecycle",
    componentType: "fileDrop",
    evidence: "media-lifecycle",
    expectedObservable:
      "Uploading an SVG or alpha PNG shape keeps whole words inside the shape only; the rotate and flip actions update the runtime media transform metadata and the renderer rebuilds the rotated/flipped alpha map; clear/remove restores unmasked placement; section or global reset removes the uploaded mask media since no default exists.",
    fixture: "1920x1080 alpha shape",
    id: "media.maskImage",
    target: "media.maskImage",
    userAction: "Upload a shape, rotate it, flip it, then clear it and reset controls.",
  }),
  controlAcceptance({
    automatedTestName: "mask invert flips placement region",
    browserTestName: "browser: mask invert flips placement region",
    componentType: "switch",
    expectedObservable: "Toggling Invert places words outside the shape instead of inside.",
    fixture: "mask loaded",
    id: "mask.invert",
    target: "mask.invert",
    userAction: "Upload a mask, then toggle Invert.",
  }),
  controlAcceptance({
    automatedTestName: "mask feather softens edge words",
    browserTestName: "browser: mask feather softens edge words",
    componentType: "slider",
    expectedObservable: "Dragging Feather fades words near the mask edge.",
    fixture: "mask loaded",
    id: "mask.feather",
    target: "mask.feather",
    userAction: "Upload a mask, then drag Feather.",
  }),
  controlAcceptance({
    ...
    componentType: "switch",
    expectedObservable:
      "Disabling Include makes PNG export transparent (alpha) and hides the live preview product background while the canvas shell stays visible; video output is not applicable for this still product.",
    fixture: "default word tide",
  ...
  controlAcceptance({
    ...
    componentType: "color",
    expectedObservable: "Changing the background color repaints the paper fill behind words.",
    fixture: "default word tide",
  ...
  controlAcceptance({
    ...
    expectedObservable: "Choosing SVG vs PNG vs JPG changes the exported file type and bytes.",
    fixture: "default word tide",
  ...
  controlAcceptance({
    ...
    evidence: "exported-bytes",
    expectedObservable:
      "Choosing 2K/4K/8K changes the exported image long-edge pixel dimensions via createToolcraftPngExportCanvas.",
    fixture: "default word tide",
    id: "export.image.resolution",
    optionCoverage: "each-visible-item",
    target: "export.image.resolution",
    userAction: "Open the Preset select and choose 8K, then export.",
  }),
  controlAcceptance({
    automatedTestName: "svg text mode switches text and outlines",
    browserTestName: "browser: svg text mode switches text and outlines",
    componentType: "select",
    evidence: "exported-bytes",
    expectedObservable:
      "SVG text is hidden while Format is PNG or JPG and becomes visible when Format is SVG; Editable exports selectable <text> elements and Outlines exports <path> geometry shaped from the bundled font binaries.",
    fixture: "svg format selected",
    id: "export.svg.text",
    optionCoverage: "each-visible-item",
    target: "export.svg.text",
    userAction: "Set Format to SVG so SVG text appears, choose Outlined paths, then export.",
  }),
  controlAcceptance({
    actionCoverage: ["export-image", "copy-image"],
    automatedTestName: "export svg actions visible for svg format",
    browserTestName: "browser: export svg actions visible for svg format",
    componentType: "panelActions",
    expectedObservable:
      "When Format is SVG, Export SVG and Copy SVG actions are visible; switching Format to PNG hides them and shows PNG actions.",
    fixture: "default word tide",
    id: "export.exportActionsSvg",
    target: "panel.actions",
    userAction: "Switch Format between SVG and PNG.",
  }),
  controlAcceptance({
    actionCoverage: ["export-image", "copy-image"],
    automatedTestName: "export png actions visible for png format",
    browserTestName: "browser: export png actions visible for png format",
    componentType: "panelActions",
    expectedObservable:
      "When Format is PNG, Export PNG and Copy PNG actions are visible; switching Format to SVG hides them and shows SVG actions.",
    fixture: "default word tide",
    id: "export.exportActionsPng",
    target: "panel.actions",
    userAction: "Switch Format between PNG and SVG.",
  }),
  controlAcceptance({
    actionCoverage: ["export-image", "copy-image"],
    automatedTestName: "export jpg actions visible for jpg format",
    browserTestName: "browser: export jpg actions visible for jpg format",
    componentType: "panelActions",
    expectedObservable:
      "When Format is JPG, Export JPG and Copy JPG actions are visible; switching Format to PNG hides them and shows PNG actions.",
    fixture: "default word tide",
    id: "export.exportActionsJpg",
    target: "panel.actions",
    userAction: "Switch Format between JPG and PNG.",
  }),
];
...
export const starterControlSectionInventory: readonly ToolcraftControlSectionInventoryEntry[] = [
  {
    entity: "Texture mode",
    groupingReason:
      "Mode, seed, and randomize decide which layout engine runs and which variation it draws.",
    targets: ["mode.render", "tide.seed", "tide.randomize"],
    title: "Texture Mode",
    workflowStage: "Choose the texture engine",
  },
  {
    entity: "Word token stream",
    groupingReason:
      "The multi-line word list is the token source every mode consumes; it renders full-width as its own block.",
    splitReason:
      "The multi-line token editor renders standalone before the ordering select so the token source reads as one full-width input block.",
    targets: ["words.list"],
    title: "Word list",
    workflowStage: "Feed the words",
  },
  {
    entity: "Word token ordering",
    groupingReason: "Ordering decides how the token stream cycles through the list per slot.",
    splitReason:
      "Ordering is the compact companion setting that follows the standalone full-width token editor block.",
    targets: ["words.order"],
    title: "Words",
    workflowStage: "Feed the words",
  },
  {
    entity: "Source image upload",
    groupingReason:
      "The uploaded photo is the luminance signal that ink mapping and tone zones sample per word.",
    splitReason:
      "The source drop zone renders standalone; its tone shaping sliders live in Ink Mapping with the rest of the ink pipeline.",
    targets: ["media.sourceImage"],
    title: "Source Image",
    workflowStage: "Load the source signal",
  },
  {
    entity: "Ink mapping",
    groupingReason:
      "Invert, fade, contrast, weight range, and sparsity form the single pipeline that turns sampled luminance into per-word ink.",
    targets: ["source.invert", "ink.fade", "source.contrast", "ink.sparsity"],
    title: "Ink Mapping",
    workflowStage: "Map ink to type",
  },
  {
    entity: "Tone zones",
    groupingReason:
      "The zone switch, band split, and per-band word lists fill tone bands with separate vocabularies.",
    targets: ["zones.enabled", "zones.split", "zones.dark", "zones.mid", "zones.light"],
    title: "Tone Zones",
    workflowStage: "Assign words to tone bands",
  },
  {
    entity: "Word grid",
    groupingReason: "Gap, jitter, and tone spacing shape the dither-mode grid geometry.",
    targets: ["grid.gap", "grid.overlap", "grid.jitter", "grid.spacingBias", "grid.spacingRange"],
    title: "Word Grid",
    workflowStage: "Tune the grid",
  },
  {
    entity: "Procedural vector field",
    groupingReason: "Pattern, direction, scale, swirl, and turbulence define the base current.",
    targets: [
      "flow.pattern",
      "flow.direction",
      "flow.frequency",
    ...
    title: "Flow Field",
    workflowStage: "Define the current",
  },
  {
    entity: "Flow guide paths",
    groupingReason: "Path edit mode, reach, strength, and path actions steer the current along splines.",
    targets: [
      "paths.editMode",
      "paths.reach",
      "paths.strength",
    ...
    title: "Flow Paths",
    workflowStage: "Steer the current",
  },
  {
    entity: "Word streams",
    groupingReason: "Density and word gap control how words pack along traced streamlines.",
    targets: ["streams.density", "streams.wordGap"],
    title: "Flow Words",
    workflowStage: "Pack words on streams",
  },
  {
    entity: "Word typography",
    groupingReason: "Font size controls Tay Big Bird uppercase word scale across every mode.",
    targets: ["type.fontSize"],
    title: "Typography",
    workflowStage: "Style the type",
  },
  {
    entity: "Ink colors",
    groupingReason:
      "Color count and the brand palette slots map ink density to one through four word colors.",
    targets: ["colors.count", "colors.color1", "colors.color2", "colors.color3", "colors.color4"],
    title: "Brand Palette",
    workflowStage: "Paint the words",
  },
  {
    entity: "Marker highlights",
    groupingReason: "Highlight words, marker color, and coverage form the marker emphasis system.",
    targets: ["highlight.words", "highlight.color", "highlight.coverage"],
    title: "Highlights",
    workflowStage: "Emphasize key words",
  },
  {
    entity: "Shape mask upload",
    groupingReason:
      "The mask drop zone supplies the alpha shape that constrains where whole words may exist.",
    splitReason:
      "The mask drop zone renders standalone before the mask shaping controls so the shape source reads as one full-width block.",
    targets: ["media.maskImage"],
    title: "Mask Image",
    workflowStage: "Constrain the shape",
  },
  {
    entity: "Shape mask shaping",
    groupingReason: "Invert and feather reshape how the uploaded alpha mask admits words.",
    splitReason:
      "Mask shaping settings follow the standalone drop zone as the compact companion controls of the same workflow stage.",
    targets: ["mask.invert", "mask.feather"],
    title: "Shape Mask",
    workflowStage: "Constrain the shape",
  },
  {
  ...
  },
  {
    ...
    groupingReason:
      "Format, resolution, and the SVG text mode are the compact export-encoding workflow set.",
    targets: ["export.image.format", "export.image.resolution", "export.svg.text"],
    title: "Image Export",
    workflowStage: "Configure the export",
  },
];
