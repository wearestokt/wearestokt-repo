...
function getRasterWorker(): Worker | null {
  ...
  if (!rasterWorker) {
    try {
      rasterWorker = new Worker("/container-yard-image-raster.worker.js");
    } catch {
      return null;
    }
  }
...
export async function loadImageElement(dataUrl: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    image.onerror = () => reject(new Error("Container Yard failed to load source image."));
    image.src = dataUrl;
  });
}
...
export async function rasterizeSourceImageOnMainThread(
  ...
  if (!context) {
    throw new Error("Container Yard source rasterization requires Canvas 2D.");
  }
...
async function rasterizeSourceImageWithWorker(
  ...
  if (!worker) {
    throw new Error("Container Yard image raster worker is unavailable.");
  }
  ...
  return new Promise((resolve, reject) => {
    const handleMessage = (event: MessageEvent<RasterWorkerResponse>) => {
      ...
      if (!data || height === undefined || width === undefined) {
        reject(new Error("Container Yard image rasterization returned empty data."));
        return;
      }
    ...
    const handleError = () => {
      cleanup();
      reject(new Error("Container Yard image raster worker failed."));
    };
